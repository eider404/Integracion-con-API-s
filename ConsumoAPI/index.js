...
function GetAll(){
    fetch('https://productos-api.fly.dev/')
        .then(res => res.json())
        .then(res => 
...
function Search(){
    ...
    fetch(`https://productos-api.fly.dev/${ search.searchId.value }`)
        .then(res => res.json())
        .then(res =>{
...
function Eliminar(){
    ...
    fetch(`https://productos-api.fly.dev/${ fmDelete.id.value }`)
        .then(res=> res.json())
        .then(res=>{
            if(res[0] == undefined) { throw new Error('¡La id no existe!') }

            //consume el delete
            fetch( `https://productos-api.fly.dev/delete/${ fmDelete.id.value }`, {
                method: 'DELETE'
            })
            .then( raw =>{
...
function Agregar(){
    ...
    //hace un get para comprobar si existe esa id
    fetch(`https://productos-api.fly.dev/${ fm_Post.id.value }`)
        .then(res=> res.json())
        .then(res=>{ 
            ...
            //consume el post
            fetch( `https://productos-api.fly.dev/new`, {
                method: 'POST',
                headers: {
...
async function Actualizar(){
    ...
    //hace un get para comprobar si existe esa id
    fetch(`https://productos-api.fly.dev/${ fm_put.id.value }`)
        .then(res=> res.json())
        .then(res=>{ 
            if(res[0] == undefined) { throw new Error('¡La id no existe!') }
            ...
            //consume el put
            fetch( `https://productos-api.fly.dev/update/${ fm_put.id.value }`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
